...
sudo -u ${local_renderd_user} psql -d gis -c "SELECT distinct osm_id,name FROM planet_osm_polygon WHERE (boundary = 'historic_parish') AND (denomination = 'anglican');" > /home/${local_filesystem_user}/data/historicchurchanglicanparishes_id.temp
